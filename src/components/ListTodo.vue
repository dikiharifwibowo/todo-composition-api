...
export default {
    ...
    setup(props, { attrs, slots, emit }) {
        const handleDoneActivity = (index) =>{ 
            emit('set-done-activity', index)
        }
        const handleDeleteTodo = (index) => {
            emit('delete-todo', index)
        }
        const handleUnCheckTodo = (index) => {
        ...
        // onMounted(() => {
        //     console.log("total", props.todos.length)
        // })
        return {
            handleDoneActivity,
            handleDeleteTodo,
        ...
        <div :class="item.isDone ? 'bg-blue-200' : 'bg-orange-200' " class="w-full relative rounded-xl overflow-auto p-4 my-2" v-for="(item, index) in todos" :key="index">
            <div class="flex justify-between">
                ...
                <div class="flex gap-4">
                    <svg @click="handleDoneActivity(index)" v-if="!item.isDone" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 cursor-pointer" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <svg @click="handleUnCheckTodo(index)" v-if="item.isDone" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 cursor-pointer" viewBox="0 0 20 20" fill="currentColor">
